...
func (dfd DeductFeeDecorator) checkDeductFee(ctx sdk.Context, sdkTx sdk.Tx, fee sdk.Coins) error {
	...
	// deduct the fees
	if !fee.IsZero() {
		if dfd.daoKeeper.FreeTxMsg(ctx, sdkTx) { 
			
			deductFee, err := dfd.daoKeeper.CalculateFee(ctx, sdkTx, fee)
			if err != nil {
				return err
			}
			if deductFee != nil { 
				err = DeductFees(dfd.bankKeeper, ctx, deductFeesFromAcc, deductFee)
				if err != nil {
					return err
				}
			}
			msgs := sdkTx.GetMsgs()
			msgsConcatString := ConcatMsgsType(msgs)
			senderAttBalance := dfd.bankKeeper.GetBalance(ctx, feePayer, core.BaseDenom)
			SenderFmBalance := dfd.bankKeeper.GetBalance(ctx, feePayer, core.GovDenom)
			events := sdk.Events{
				sdk.NewEvent(
					sdk.EventTypeTx,
					sdk.NewAttribute(sdk.AttributeKeyFee, fee.String()),
					sdk.NewAttribute(sdk.AttributeKeyFeePayer, deductFeesFrom.String()),
					sdk.NewAttribute(daotypes.AttributeKeyFeeSingle, deductFee.AmountOf(core.BaseDenom).String()),
					sdk.NewAttribute(sdk.AttributeKeySender, feePayer.String()),
					sdk.NewAttribute(daotypes.AttributeKeyMemo, msgsConcatString),
					sdk.NewAttribute(daotypes.AttributeKeySenderBalances, senderAttBalance.String()+","+SenderFmBalance.String()),
				),
			}
