...
func (this *EvmClient) GetContractCode(contract string) (string, error) {
    ...
    resBytes, _, err := clientCtx.QueryWithData("custom/contract/"+types.QueryContractCode, bz)
    if err != nil {
...
func (this *EvmClient) GetNftInfo(address, contract string) ([]types.NftInfo, error) {
    ...
    resBytes, _, err := clientCtx.QueryWithData("custom/contract/"+types.QueryNft, bz)
    if err != nil {
        log.WithError(err).Error("QueryWithData")
        return nil, err
    }
...
func (this *EvmClient) GetNftContractAddress() (string, error) {
    log := core.BuildLog(core.GetStructFuncName(this), core.LmChainClient)
    resBytes, _, err := clientCtx.QueryWithData("custom/contract/"+types.QueryNftContractAddress, nil)
    if err != nil {
        log.WithError(err).Error("QueryWithData")
        return "", err
    }
