...
//func (k Keeper) initializeValidator(ctx sdk.Context, val types.ValidatorI) {
...
func (k Keeper) IncrementValidatorPeriod(ctx sdk.Context, val types.ValidatorI) uint64 {
	// fetch current rewards
	logs := core.BuildLog(core.GetFuncName(), core.LmChainPledgeKeeper)

	rewards := k.GetValidatorCurrentRewards(ctx, val.GetOperator())
	logs.Debug("rewards.Rewards", rewards.Rewards)
	logs.Debug("rewards.Period", rewards.Period)
	// calculate current ratio
	var current sdk.DecCoins
	if val.GetTokens().IsZero() {
		logs.Debug("val has no tokens")
		// can't calculate ratio for zero-token validators ergo we instead add to the community pool
		...
		logs.Debug("val has tokens:", val.GetTokens())
