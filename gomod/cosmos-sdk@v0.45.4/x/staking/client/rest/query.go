...
func registerQueryRoutes(clientCtx client.Context, r *mux.Router) {
	...
	// Get the current node validator info
	r.HandleFunc(
		"/staking/node/validator/{delegatorAddr}",
		nodeValidatorInfoHandlerFn(clientCtx),
	).Methods("GET")
}

// HTTP request handler to query a node validator info
func nodeValidatorInfoHandlerFn(clientCtx client.Context) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		validatorAddress := sdk.ValAddress(delAddr)
		...
		validatorInfo := &types.ValidatorInfor{
			...
			ValidatorOperAddr: validatorAddress.String(),
		}
