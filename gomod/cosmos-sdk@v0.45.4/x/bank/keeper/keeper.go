...
type Keeper interface {
	...
	DelegateCoinsFromAccountToModuleForPledge(ctx sdk.Context, fromAddr, senderAddr sdk.AccAddress, recipientModule string, amt sdk.Coins) error
	types.QueryServer
}
...
func (k BaseKeeper) DelegateCoinsForPledge(ctx sdk.Context, fromAddr, delegatorAddr, moduleAccAddr sdk.AccAddress, amt sdk.Coins) error {
	moduleAcc := k.ak.GetAccount(ctx, moduleAccAddr)
	if moduleAcc == nil {
		return sdkerrors.Wrapf(sdkerrors.ErrUnknownAddress, "module account %s does not exist", moduleAccAddr)
	}
	...
	for _, coin := range amt {
		...
		balances = balances.Add(balance)
		err := k.setBalance(ctx, fromAddr, balance.Sub(coin))
		if err != nil {
			return err
		}
	}
...
func (k BaseKeeper) DelegateCoinsFromAccountToModuleForPledge(
	ctx sdk.Context, fromAddr, senderAddr sdk.AccAddress, recipientModule string, amt sdk.Coins,
) error {
	...
	return k.DelegateCoinsForPledge(ctx, fromAddr, senderAddr, recipientAcc.GetAddress(), amt)
}
